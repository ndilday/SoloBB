...
| Post-match sequence | tested | Smoke section: post-match campaign lifecycle | Records scheduled results, game Fan Factors, winnings, Dedicated Fans changes, casualty roster updates, old Miss Next Game cleanup, treasury spend, MVPs, and SPP awards. The MVP is now randomly selected (BB2020) from eligible non-journeyman players via the dice roller. Winnings now follow BB2020: each team's game Fan Factor (Dedicated Fans + D3) is rolled at kickoff and stored on the match, the two are summed into Fan Attendance, and each coach receives `(Fan Attendance / 2 + touchdowns scored) * 10,000`. The Dedicated Fans characteristic is updated by the BB2020 dice rule (win: D6 ≥ current → +1 to a max of 6; loss: D6 < current → −1 to a min of 1; draw: no change), and the resulting per-team change is stored on the scheduled match result. |
...
| Dump-Off | tested | Smoke section: hand-offs, passing, and interference | A block or blitz against a Dump-Off carrier raises a `PendingDumpOff` interrupt offering a Quick Pass before the block dice; the held block resumes after the pass (and any interception/reroll it spawns) settles. |
| Running Pass | tested | Smoke section: hand-offs, passing, and interference | Allows movement continuation after passing. |
| On the Ball | tested | Smoke sections: hand-offs/passing/interference and kickoff/weather/events | Both BB2020 windows are wired as `PendingOnTheBall` interrupts. Pass window: a declared pass lets a defender with the skill move up to three squares (each closer to the pass target) before any roll, then the pass resumes. Kickoff window: after the kick but before the kick-off event roll, the receiving team may move a skill player up to three squares (staying in their own half), then the kickoff resumes. |
...
| Diving Tackle | tested | Movement skills section | Offered as a defender choice after a successful Dodge or Leap that leaves the tackler's zone, applying -2 (and placing the tackler prone); the dodging team may then reroll. |
