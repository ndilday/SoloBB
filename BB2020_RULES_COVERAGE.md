...
| Team drafting | tested | Smoke section: league creation, roster validation, and persistence | Covers roster limits, team value, treasury, rerolls, fan factor, staff, and apothecaries. |
...
| Inducements | tested | Smoke section: match creation, setup, and persistence | Ruleset data includes inducements. Pre-game service computes petty cash, validates budgets, passes bribes into match state, and can add eligible data-backed star players to match-only teams. |
| Journeymen | tested | Smoke section: match creation, setup, and persistence | Adds match-only Loner journeymen to reach the ruleset player count without changing league rosters. |
| Post-match sequence | tested | Smoke section: post-match campaign lifecycle | Records scheduled results, applies casualty roster updates, clears old Miss Next Game status, applies winnings/treasury spend, MVPs, SPP awards, and fan-factor changes. End-of-season redraft remains pending. |
| Player advancement | tested | Smoke section: post-match campaign lifecycle | Selected and deterministic random skill advancement purchases spend SPP and increase team value. Advancement UI and fuller BB2020 random-table behavior are still pending. |
